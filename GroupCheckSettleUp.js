...
export default function GroupCheckSettleUpPayment({ route, navigation }) {
    ...
    const getSettleUpPayment = async () => {
        console.log(group_expense_id);
        try {
            const response = await axios.get("http://192.168.0.112/expensepal_api/getGroupSettleUpPayment.php", {
                params: {
    ...
    const handleAccept = async (paymentId) => {
        ...
        try {
            const response = await axios.post("http://192.168.0.112/expensepal_api/updateGroupSettleUpPayment.php", {
                id: paymentId,
                status: true,
            })
            console.log(response.data);
